...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 12-09-2025 a las 04:43:19
...
CREATE TABLE `abono_venta` (
  ...
  `fecha` date NOT NULL,
  `mes_correspondiente` int(11) NOT NULL DEFAULT 1,
  `año_correspondiente` int(11) NOT NULL DEFAULT year(curdate()),
...
(3, 'cliente', 'Escobar', 2, '12345678', '12348765', 'Cliente@gmail.com', 'PIPIPI');
...
INSERT INTO `factura_venta` (`id_factura_venta`, `id_cliente`, `id_vende`, `id_product`, `fecha`, `hora`, `Precio_Mensual`, `Cuotas`, `direccion`, `interes_aplicado`, `total`, `es_credito`, `id_cobrador`, `estado_pago`, `monto_abonado`) VALUES
(2, 1, 1, 4, '2025-09-11', '14:43:09', 25, b'1', 'Santa Teresa de las flores :D', 18.00, 589.99, 1, 2, 'pendiente', 0.00),
(4, 1, 1, 3, '2025-08-27', '17:15:38', 4, b'1', 'Santa Teresa de las flores :D', 8.00, 26.99, 1, 1, 'pendiente', 0.00),
(5, 1, 1, 4, '2025-09-07', '13:33:11', 175, b'1', 'San Salvador, apopa madre tierra', 5.00, 524.99, 1, 2, 'pendiente', 0.00),
(6, 1, 1, 3, '2025-09-07', '20:37:32', 4, b'1', 'Santa Teresa de las flores :D', 8.00, 26.99, 1, 1, 'pendiente', 0.00),
(8, 3, 1, 4, '2025-09-11', '20:09:11', 47, b'1', 'San Salvador, apopa madre tierra', 12.00, 559.99, 1, NULL, 'pendiente', 0.00);
...
(4, 'Omar Enrique Ventura', NULL, 'omar@gmail.com', '12345678', 1, 4),
(6, 'Maria Gonzales', NULL, 'maria@gmail.com', '12345678', 1, 4),
(8, 'vendedor', NULL, 'vendedor@gmail.com', '12345678', 1, 3),
(9, 'cliente', NULL, 'Cliente@gmail.com', '12345678', 1, 2);
...
(2, 'vendedor', 'Castillo', '76379499', 3, 4);

-- --------------------------------------------------------

--
-- Estructura Stand-in para la vista `vista_cobros`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `vista_cobros` (
`id_factura_venta` int(11)
,`fecha_venta` date
,`total` decimal(10,2)
,`cuotas` bit(1)
,`precio_mensual` decimal(10,0)
,`estado_pago` enum('pendiente','parcial','pagado')
,`monto_abonado` decimal(10,2)
,`saldo_pendiente` decimal(11,2)
,`id_cliente` int(11)
...
,`id_cobrador` int(11)
,`cobrador_nombre` varchar(20)
,`cobrador_apellido` varchar(20)
);
...
--
-- Estructura para la vista `vista_cobros`
--
DROP TABLE IF EXISTS `vista_cobros`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista_cobros`  AS SELECT `fv`.`id_factura_venta` AS `id_factura_venta`, `fv`.`fecha` AS `fecha_venta`, `fv`.`total` AS `total`, `fv`.`Cuotas` AS `cuotas`, `fv`.`Precio_Mensual` AS `precio_mensual`, `fv`.`estado_pago` AS `estado_pago`, `fv`.`monto_abonado` AS `monto_abonado`, `fv`.`total`- `fv`.`monto_abonado` AS `saldo_pendiente`, `c`.`id_cliente` AS `id_cliente`, `c`.`nombre` AS `cliente_nombre`, `c`.`apellido` AS `cliente_apellido`, `c`.`tel` AS `cliente_telefono`, `c`.`direccion` AS `cliente_direccion`, `p`.`nombre` AS `producto_nombre`, `cb`.`id_cobrador` AS `id_cobrador`, `cb`.`nombre` AS `cobrador_nombre`, `cb`.`apellido` AS `cobrador_apellido` FROM (((`factura_venta` `fv` join `cliente` `c` on(`fv`.`id_cliente` = `c`.`id_cliente`)) join `producto` `p` on(`fv`.`id_product` = `p`.`id_product`)) left join `cobrador` `cb` on(`fv`.`id_cobrador` = `cb`.`id_cobrador`)) WHERE `fv`.`es_credito` = 1 AND `fv`.`id_cobrador` is not null ;
...
--
ALTER TABLE `factura_venta`
  MODIFY `id_factura_venta` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=9;
...
--
ALTER TABLE `vendedor`
  MODIFY `id_vende` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;
